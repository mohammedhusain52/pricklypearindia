...
class Pisces_Breadcrumbs {
	...
	function __construct( $args = array() ) {
		...
		$defaults = array(
			...
			'page_title_format' => '<header><h3 class="page-title h3">%s</h3></header>',
			...
			'labels'            => array(),
			'date_labels'       => array(),
			'action'            => 'pisces/action/breadcrumbs/render_html',
			'css_namespace'     => array(),
			'path_type'         => 'full', // minified , full
			'post_taxonomy'     => apply_filters(
				'pisces/filter/breadcrumbs/trail_taxonomies',
				array(
					'post'      => 'category',
					'product'	=> 'product_cat'
				)
		...
		$this->args = apply_filters( 'pisces/filter/breadcrumbs/args', wp_parse_args( $args, $defaults ) );
	...
	public function get_trail() {
		...
		$wrapper_classes = apply_filters( 'pisces/filter/breadcrumbs/wrapper_classes', $wrapper_classes );
	...
	public function get_items() {
		...
		$breadcrumb = apply_filters( 'pisces/filter/breadcrumbs/trail', $breadcrumb, $this->args );
	...
	public function get_title() {
		...
		$title = apply_filters(
			'pisces/filter/page_title',
			sprintf( $this->args['page_title_format'], $this->_page_title ), $this->args
		);
	...
	public function default_labels() {
		...
		$labels = array(
			'browse'              => esc_html_x( 'Browse:', 'front-view', 'pisces' ),
			'home'                => $this->home_title(),
			'error_404'           => esc_html_x( '404 Not Found', 'front-view', 'pisces' ),
			'archives'            => esc_html_x( 'Archives', 'front-view', 'pisces' ),
			'search'              => esc_html_x( 'Search results for &#8220;%s&#8221;', 'front-view', 'pisces' ),
			'paged'               => esc_html_x( 'Page %s', 'front-view', 'pisces' ),
			'archive_minute'      => esc_html_x( 'Minute %s', 'front-view', 'pisces' ),
			'archive_week'        => esc_html_x( 'Week %s', 'front-view', 'pisces' ),
	...
	public function home_title() {
		...
		$title            = esc_html_x( 'Home', 'front-view', 'pisces' );
		$use_custom_title = apply_filters( 'pisces/filter/breadcrumbs/custom_home_title', false );
	...
	public function build_trail() {
		...
		$custom_trail = apply_filters( 'pisces/filter/breadcrumbs/custom_trail', false, $this->args );
		 ...
		 */
		$this->items = apply_filters( 'pisces/filter/breadcrumbs/items', $this->items, $this->args, $this );
	 ...
	 */
	public function _add_item( $format = 'link_format', $label, $url = '', $class = '', $prepend = false, $is_page_title = false ) {
		...
		if ( 'target_format' == $format ) {
			if(!isset($this->_count_target_format)){
				$this->_count_target_format = 1;
			}else{
				$this->_count_target_format++;
			}

			$this->page_title = $label;
			if($is_page_title){
				$this->_page_title = $label;
			}
		}
		...
		if ( true === $prepend ) {
			...
			if((isset($this->_count_target_format) && $this->_count_target_format == 2)){
	...
	function add_path_parents( $path ) {
		...
		// process default permalinks by own way
		if ( in_array( $path, array( 'tag', 'category', 'author' ) ) ) {
			return;
		}
	...
	public function add_front_page() {
		...
		if ( is_paged() || ( is_singular() && 1 < get_query_var( 'page' ) ) ) {
			...
			$this->_add_item( 'target_format', $label, '', '', false, true );
		}
	...
	public function add_post_terms( $post_id ) {
		...
		if ( ! empty( $this->args['post_taxonomy'][ $post_type ] ) ) {
			add_filter('get_object_terms', 'pisces_exclude_demo_term_in_category');
			$post_terms = wp_get_post_terms( $post_id, $this->args['post_taxonomy'][ $post_type ] );
			remove_filter('get_object_terms', 'pisces_exclude_demo_term_in_category');
	...
	public function add_post_type_archive_items() {
		...
		if ( is_paged() ) {
            ...
            if($post_type_object->name == 'product' && function_exists('wc_get_page_id')){
                $label = apply_filters('woocommerce_page_title', get_the_title( wc_get_page_id('shop') ) );
            }
	...
	public function add_day_archive_items() {
		...
		$this->_add_item( 'target_format', $day, '', '', false, true );
	...
	public function add_week_archive_items() {
		...
		$this->_add_item( 'target_format', $week, '', '', false, true );
	...
	public function add_month_archive_items() {
		...
		$this->_add_item( 'target_format', $month, '', '', false, true );
	}
	...
	public function add_year_archive_items() {
		...
		$this->_add_item( 'target_format', $year, '', '', false, true );
	...
	public function add_default_archive_items() {
		...
		$this->_add_item( 'target_format', $this->args['labels']['archives'], '', '', false, true );
	...
	public function add_404_items() {
		...
		$this->_add_item( 'target_format', $this->args['labels']['error_404'], '', '', false, true );
	...
	public function prepare_label( $label, $fallback = false ) {
		...
		if ( false !== $fallback ) {
			$fallback = sprintf(
				apply_filters( 'pisces/filter/breadcrumbs/icon_fallback', '<span class="hidden">%s</span>', $fallback ),
				esc_attr( $fallback )
			);
		}
		...
		// Return default icon
		return sprintf(
			apply_filters( 'pisces/filter/breadcrumbs/default_icon_format', '<span class="%1$s">%2$s</span>', $label, $fallback ),
