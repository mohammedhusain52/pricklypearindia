...
class Pisces {
    ...
    private static $original_option_name = 'pisces_options';
    ...
    private function __construct() {
        ...
        wp_cache_add_non_persistent_groups( 'pisces' );
        ...
        $this->settings       = Pisces_Setting::get_instance();
        $this->init           = new Pisces_Init();
        $this->template       = new Pisces_Template();
        $this->blog           = new Pisces_Blog();
        $this->images         = new Pisces_Images();
        $this->layout         = new Pisces_Layout();
        $this->favorite       = Pisces_FavoritePosts::get_instance();

        add_action( 'wp', array( $this, 'set_page_id' ) );
        add_action( 'wp', array( $this, 'set_current_context' ) );
        add_action( 'wp', array( $this, 'build_crumbs'));

        new Pisces_Widget_CssClass();

        do_action( 'pisces_core_loaded' );
    }
    ...
    public function build_crumbs(){
        ...
        $this->breadcrumbs = new Pisces_Breadcrumbs( $args );
    }

    public function set_is_updating() {
        if ( ! self::$is_updating && $_GET && isset( $_GET['pisces_update'] ) && '1' == $_GET['pisces_update'] ) {
    ...
    public static function multilingual_options() {
        // Set the self::$lang.
        if ( ! in_array( Pisces_Multilingual::get_active_language(), array( '', 'en', 'all' ) ) ) {
            self::$lang = '_' . Pisces_Multilingual::get_active_language();
        }
