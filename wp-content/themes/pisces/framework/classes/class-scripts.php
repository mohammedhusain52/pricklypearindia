...
class Pisces_Scripts
{
    ...
    public function __construct()
    {
        ...
        add_action('pisces/action/head', array( $this, 'get_custom_css_from_setting'));
        add_action('pisces/action/head', array( $this, 'add_custom_header_js' ), 100 );
        add_action('pisces/action/footer', array( $this, 'add_custom_footer_js' ), 100 );
    }
    ...
    public function enqueue_scripts()
    {
        ...
        $script_min_path = apply_filters('pisces/filter/js_load_min_file', 'min/');
        ...
        wp_enqueue_style('font-awesome', Pisces::$template_dir_url . '/assets/css/font-awesome.min.css', array(), null);
        wp_enqueue_style('animate-css', Pisces::$template_dir_url . '/assets/css/animate.min.css', array(), null);
        wp_enqueue_style('pisces-theme', get_template_directory_uri() . '/style.css', array('font-awesome'), null);
        ...
        switch ($font_source) {
            case '1':
                wp_enqueue_style('pisces-google_fonts', $this->get_google_font_url(), array(), null);
                break;
            case '2':
                wp_enqueue_style('pisces-font_google_code', $this->get_google_font_code_url(), array(), null);
                break;
            case '3':
                wp_enqueue_script('pisces-font_typekit', $this->get_google_font_typekit_url(), array(), null);
                wp_add_inline_script( 'pisces-font_typekit', 'try{ Typekit.load({ async: true }) }catch(e){}' );
                break;
        }

        wp_enqueue_script( 'respond', Pisces::$template_dir_url . '/assets/js/enqueue/min/respond.js');
        ...
        wp_register_script( 'pisces-modernizr-custom', Pisces::$template_dir_url . '/assets/js/enqueue/min/modernizr-custom.js', array('jquery'), '2.6.2', true);

        $fullpage_config = array();
        $js_require = array('jquery','pisces-modernizr-custom');

        if (in_array('is_page', Pisces()->get_current_context())) {
            $fp_metadata = Pisces()->settings->get_post_meta(get_the_ID());

            if (Pisces()->layout->get_site_layout() == 'col-1c' && (!empty($fp_metadata['enable_fp']) && $fp_metadata['enable_fp'] == 'yes')) {
                ...
                if(!empty($fp_metadata['fp_navigation']) && $fp_metadata['fp_navigation'] != 'off'){
                    if($section_nav_file != 'number'){
                        wp_enqueue_style('pisces-fullpage-mav', Pisces::$template_dir_url . '/assets/css/fullpage/nav/section/'.$section_nav_file.'.css', array('pisces-theme'), null);
                    }
                }
                if(!empty($fp_metadata['fp_slidenavigation']) && $fp_metadata['fp_slidenavigation'] != 'off'){
                    ...
                    if($slide_nav_file != 'number'){
                        wp_enqueue_style('pisces-fullpage-mav', Pisces::$template_dir_url . '/assets/css/fullpage/nav/slide/'.$slide_nav_file.'.css', array('pisces-theme'), null);
                    }
                }
                ...
                if (substr($fp_easing, 0, 3) == 'js_') {
                    wp_register_script('pisces-easings', Pisces::$template_dir_url . '/assets/js/enqueue/min/jquery.easings.js', array('jquery'), null, true);
                    $fullpage_js_require[] = 'pisces-easings';
                }
                if ($fp_scrolloverflow == 'yes') {
                    wp_register_script('pisces-scrolloverflow', Pisces::$template_dir_url . '/assets/js/enqueue/min/jquery.scrolloverflow.js', array('jquery'), null, true);
                    $fullpage_js_require[] = 'pisces-scrolloverflow';
                }
                wp_register_script('pisces-fullpage-parallax', Pisces::$template_dir_url . '/assets/js/enqueue/min/jquery.fullpage.parallax.js', array('jquery'), null, true);
                $fullpage_js_require[] = 'pisces-fullpage-parallax';

                wp_register_script('pisces-fullpage', Pisces::$template_dir_url . '/assets/js/enqueue/min/jquery.fullpage.extensions.js', $fullpage_js_require, null, true);
                $js_require[] = 'pisces-fullpage';
        ...
        if(apply_filters('pisces/filter/force_enqueue_js_external', true)){
            wp_register_script('pisces-plugins', Pisces::$template_dir_url . '/assets/js/plugins/min/plugins-full.js', array('jquery'), null, true);
            $js_require[] = 'pisces-plugins';
        }

        wp_enqueue_script('pisces-theme', Pisces::$template_dir_url . '/assets/js/'.$script_min_path.'app.js', $js_require, $theme_version, true);

        wp_localize_script('pisces-theme', 'la_theme_config', apply_filters('pisces/filter/global_message_js', array(
            'security' => array(
                'favorite_posts' => wp_create_nonce('favorite_posts'),
                'instagram_token' => esc_attr(Pisces()->settings->get('instagram_token'))
            ),
            'fullpage' => $fullpage_config,
            'product_single_design' => esc_attr(Pisces()->settings->get('woocommerce_product_page_design', 1)),
            'i18n' => array(
                'backtext' => esc_attr_x('Back', 'front-view', 'pisces'),
                'compare' => array(
                    'view' => esc_attr_x('View List Compare', 'front-view', 'pisces'),
                    'success' => esc_attr_x('has been added to comparison list.', 'front-view', 'pisces'),
                    'error' => esc_attr_x('An error occurred ,Please try again !', 'front-view', 'pisces')
                ),
                'wishlist' => array(
                    'view' => esc_attr_x('View List Wishlist', 'front-view', 'pisces'),
                    'success' => esc_attr_x('has been added to your wishlist.', 'front-view', 'pisces'),
                    'error' => esc_attr_x('An error occurred ,Please try again !', 'front-view', 'pisces')
                ),
                'addcart' => array(
                    'view' => esc_attr_x('View Cart', 'front-view', 'pisces'),
                    'success' => esc_attr_x('has been added to your cart', 'front-view', 'pisces'),
                    'error' => esc_attr_x('An error occurred ,Please try again !', 'front-view', 'pisces')
                ),
                'global' => array(
                    'error' => esc_attr_x('An error occurred ,Please try again !', 'front-view', 'pisces'),
                    'comment_author' => esc_attr_x('Please enter Name !', 'front-view', 'pisces'),
                    'comment_email' => esc_attr_x('Please enter Email Address !', 'front-view', 'pisces'),
                    'comment_rating' => esc_attr_x('Please select a rating !', 'front-view', 'pisces'),
                    'comment_content' => esc_attr_x('Please enter Comment !', 'front-view', 'pisces'),
                    'continue_shopping' => esc_attr_x('Continue Shopping', 'front-view', 'pisces'),
                )
            ),
            'popup' => array(
                'max_width' => esc_attr(Pisces()->settings->get('popup_max_width', 790)),
                'max_height' => esc_attr(Pisces()->settings->get('popup_max_height', 430))
            ),
            'js_path'       => esc_attr(Pisces::$template_dir_url . '/assets/js/plugins/' . $script_min_path),
            'theme_path'    => esc_attr(Pisces::$template_dir_url . '/'),
            'ajax_url'      => esc_attr(admin_url('admin-ajax.php')),
            'mm_mb_effect' => esc_attr(Pisces()->settings->get('mm_mb_effect', 1)),
            'header_height' => array(
                'desktop' => array(
                    'normal' => esc_attr(str_replace('px', '', Pisces()->settings->get('header_height', 100))),
                    'sticky' => esc_attr(str_replace('px', '', Pisces()->settings->get('header_sticky_height', 80)))
                ),
                'tablet' => array(
                    'normal' => esc_attr(str_replace('px', '', Pisces()->settings->get('header_sm_height', 100))),
                    'sticky' => esc_attr(str_replace('px', '', Pisces()->settings->get('header_sm_sticky_height', 80)))
                ),
                'mobile' => array(
                    'normal' => esc_attr(str_replace('px', '', Pisces()->settings->get('header_mb_height', 100))),
                    'sticky' => esc_attr(str_replace('px', '', Pisces()->settings->get('header_mb_sticky_height', 80)))
                )
            )
        )));
        ...
        wp_add_inline_style('pisces-theme', Pisces_Helper::compress_text($this->dynamic_css(), true));

        $asset_font_without_domain = apply_filters('pisces/filter/assets_font_url', untrailingslashit(get_template_directory_uri()));
        wp_add_inline_style(
            "font-awesome",
            "@font-face{
        ...
        wp_add_inline_style(
            ...
            "@font-face{
                font-family: 'pisces';
                src: url('{$asset_font_without_domain}/assets/fonts/pisces.eot');
                src: url('{$asset_font_without_domain}/assets/fonts/pisces.eot') format('embedded-opentype'),
                     url('{$asset_font_without_domain}/assets/fonts/pisces.woff') format('woff'),
                     url('{$asset_font_without_domain}/assets/fonts/pisces.ttf') format('truetype'),
                     url('{$asset_font_without_domain}/assets/fonts/pisces.svg') format('svg');
                font-weight:normal;
                font-style:normal
            }"
        );
    ...
    {
        ob_start();
        include Pisces::$template_dir_path . '/framework/functions/additional_css.php';
        include Pisces::$template_dir_path . '/framework/functions/dynamic_css.php';
        return ob_get_clean();
    }

    public function get_custom_css_from_setting(){
        if( $la_custom_css = Pisces()->settings->get('la_custom_css') ){
            printf( '<%1$s id="pisces-extra-custom-css" type="%3$s">%2$s</%1$s>', 'style', $la_custom_css, 'text/css' );
        }
    }
    ...
    */
    ...
    {
        return (in_array($handle, array('pisces-theme'))) ? preg_replace('/(><\/[a-zA-Z][^0-9](.*)>)$/', ' async $1 ', $tag) : $tag;
    }
    ...
    public function get_gfont_from_setting(){
        $array = array();
        $main_font = Pisces()->settings->get('main_font');
        $secondary_font = Pisces()->settings->get('secondary_font');
        $highlight_font = Pisces()->settings->get('highlight_font');
    ...
    public function get_google_font_url(){
        ...
        $main_font = (array) Pisces()->settings->get('main_font');
        $secondary_font = (array) Pisces()->settings->get('secondary_font');
        $highlight_font = (array) Pisces()->settings->get('highlight_font');
    ...
    public function get_google_font_code_url() {
        $fonts_url = '';
        $_font_code = Pisces()->settings->get('font_google_code', '');
        if(!empty($_font_code)){
    ...
    public function get_google_font_typekit_url(){
        $fonts_url = '';
        $_api_key = Pisces()->settings->get('font_typekit_kit_id', '');
        if(!empty($_api_key)){
    ...
    public function add_custom_header_js(){
        printf( '<%1$s %2$s>try{ %3$s }catch (ex){}</%1$s>', 'script', 'type="text/javascript"', Pisces()->settings->get('header_js') );
    }

    public function add_custom_footer_js(){
        printf( '<%1$s %2$s>try{ %3$s }catch (ex){}</%1$s>', 'script', 'type="text/javascript"', Pisces()->settings->get('footer_js') );
    }
