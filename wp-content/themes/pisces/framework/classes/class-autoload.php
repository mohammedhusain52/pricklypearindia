...
/**
 * The Autoloader class for Pisces.
 */
class Pisces_Autoload {
    ...
    public function __construct( $args = array() ) {
        ...
        if ( empty( self::$transient_name ) ) {
            self::$transient_name = 'pisces_autoloader_paths_' . md5( __FILE__ );
        }
    ...
    protected function get_path( $class_name ) {
        ...
        $paths = array();
        if ( 0 === stripos( $class_name, 'Pisces' ) ) {
            $path     = wp_normalize_path( Pisces::$template_dir_path . '/framework/classes/' );

            $class_name = str_replace( array( 'Pisces_' ), '', $class_name );
