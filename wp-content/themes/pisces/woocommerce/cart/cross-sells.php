...
<?php
	...
	$columns = shortcode_atts(
		...
		Pisces()->settings->get('crosssell_products_columns')
	);
	...
	$slide_configs 	= Pisces_Helper::get_slick_slider_config(array_merge(array('arrows' => true), $columns));

	$title = Pisces()->settings->get('crosssell_product_title') ? Pisces()->settings->get('crosssell_product_title') : _x( 'You may be interested in&hellip;', 'front-view', 'pisces' );
	$sub_title = Pisces()->settings->get('crosssell_product_subtitle') ? Pisces()->settings->get('crosssell_product_subtitle') : '';
